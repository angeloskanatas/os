...
void child_exit(struct thread_info_struct *thr)
{
	...
	pthread_mutex_lock(&thr->kg->mutex);
	--(thr->kg->vc);
	ret=pthread_cond_broadcast(&thr->kg->cond); //notify children to enter and teachers to
	if(ret) {                                   //exit without prioritizing (one cond var)!
...
void teacher_enter(struct thread_info_struct *thr)
{
	...
	pthread_mutex_lock(&thr->kg->mutex);
	++(thr->kg->vt);                            //notify children to enter and teachers to
	ret=pthread_cond_broadcast(&thr->kg->cond); //exit without prioritizing (one cond var)!
	if(ret) {
		perror_pthread(ret, "pthread_cond_broadcast");
		exit(1);
	}
